...
def load_demo():
    ...
    samples = []
    for fname, mod, kind in specs:
        with db() as con:
            existing = con.execute('SELECT * FROM datasets WHERE filename=?', (fname,)).fetchone()
        if existing:
        ...
        make_preview(dest, preview)
        md = read_raster(dest)['meta']
        md['period_label'] = 'July 2025' if kind == 'sar_before' else 'August 2025' if kind in ('sar_after', 'optical_assam') else None
        md['demo_data'] = True
        with db() as con:
            con.execute('INSERT INTO datasets VALUES(?,?,?,?,?,?,?)', (did, fname, str(dest), str(preview), mod, json.dumps(md), now()))
        samples.append(dataset_row(get_dataset(did)))
    return samples
